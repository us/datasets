# coding=utf-8
# Copyright 2019 The TensorFlow Datasets Authors.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""Wikipedia dataset containing cleaned articles of all languages."""

from __future__ import absolute_import
from __future__ import division
from __future__ import print_function

import json
import math
import re
import xml.etree.cElementTree as etree

from absl import logging
import six
import tensorflow as tf
from tensorflow_datasets.core import api_utils
from tensorflow_datasets.core.lazy_imports import lazy_imports
import tensorflow_datasets.public_api as tfds

_DESCRIPTION = (
    "Wikipedia dataset containing cleaned articles of all languages. "
    "The datasets are built from the Wikipedia dump "
    "(https://dumps.wikimedia.org/) with one split per language. Each "
    "example contains the content of one full Wikipedia article with cleaning "
    "to strip markdown and unwanted sections (references, etc.).")

# Source: https://en.wikipedia.org/wiki/List_of_Wikipedias (accessed 3/1/2019)
_WIKIPEDIA_LANGUAGES = [
    "aa", "ab", "ace", "ady", "af", "ak", "als", "am", "an", "ang", "ar", "arc",
    "arz", "as", "ast", "atj", "av", "ay", "az", "azb", "ba", "bar", "bat-smg",
    "bcl", "be", "be-x-old", "bg", "bh", "bi", "bjn", "bm", "bn", "bo", "bpy",
    "br", "bs", "bug", "bxr", "ca", "cbk-zam", "cdo", "ce", "ceb", "ch", "cho",
    "chr", "chy", "ckb", "co", "cr", "crh", "cs", "csb", "cu", "cv", "cy", "da",
    "de", "din", "diq", "dsb", "dty", "dv", "dz", "ee", "el", "eml", "eo", "es",
    "et", "eu", "ext", "fa", "ff", "fi", "fiu-vro", "fj", "fo", "fr", "frp",
    "frr", "fur", "fy", "ga", "gag", "gan", "gd", "gl", "glk", "gn", "gom",
    "gor", "got", "gu", "gv", "ha", "hak", "haw", "he", "hi", "hif", "ho", "hr",
    "hsb", "ht", "hu", "hy", "hz", "ia", "id", "ie", "ig", "ii", "ik", "ilo",
    "inh", "io", "is", "it", "iu", "ja", "jam", "jbo", "jv", "ka", "kaa", "kab",
    "kbd", "kbp", "kg", "ki", "kj", "kk", "kl", "km", "kn", "ko", "koi", "kr",
    "krc", "ks", "ksh", "ku", "kv", "kw", "ky", "la", "lad", "lb", "lbe", "lez",
    "lfn", "lg", "li", "lij", "lmo", "ln", "lo", "lrc", "lt", "ltg", "lv",
    "mai", "map-bms", "mdf", "mg", "mh", "mhr", "mi", "min", "mk", "ml", "mn",
    "mr", "mrj", "ms", "mt", "mus", "mwl", "my", "myv", "mzn", "na", "nah",
    "nap", "nds", "nds-nl", "ne", "new", "ng", "nl", "nn", "no", "nov", "nrm",
    "nso", "nv", "ny", "oc", "olo", "om", "or", "os", "pa", "pag", "pam", "pap",
    "pcd", "pdc", "pfl", "pi", "pih", "pl", "pms", "pnb", "pnt", "ps", "pt",
    "qu", "rm", "rmy", "rn", "ro", "roa-rup", "roa-tara", "ru", "rue", "rw",
    "sa", "sah", "sat", "sc", "scn", "sco", "sd", "se", "sg", "sh", "si",
    "simple", "sk", "sl", "sm", "sn", "so", "sq", "sr", "srn", "ss", "st",
    "stq", "su", "sv", "sw", "szl", "ta", "tcy", "te", "tet", "tg", "th", "ti",
    "tk", "tl", "tn", "to", "tpi", "tr", "ts", "tt", "tum", "tw", "ty", "tyv",
    "udm", "ug", "uk", "ur", "uz", "ve", "vec", "vep", "vi", "vls", "vo", "wa",
    "war", "wo", "wuu", "xal", "xh", "xmf", "yi", "yo", "za", "zea", "zh",
    "zh-classical", "zh-min-nan", "zh-yue", "zu"]


class WikipediaConfig(tfds.core.BuilderConfig):
  """BuilderConfig for Wikipedia."""

  @api_utils.disallow_positional_args
  def __init__(self, date=None, **kwargs):
    """BuilderConfig for Wikipedia.

    Args:
      date: string, date of the Wikipedia dump in YYYYMMDD format. A list of
        available dates can be found at https://dumps.wikimedia.org/enwiki/.
      **kwargs: keyword arguments forwarded to super.
    """
    super(WikipediaConfig, self).__init__(
        name="wikipedia_ogm_%s" % date,
        description=(
            "Wikipedia dataset split by language, parsed from %s dump" % date),
        **kwargs)
    self.date = date


class Wikipedia(tfds.core.GeneratorBasedBuilder):
  """Wikipedia dataset for the OGM moonshot."""
  # Use mirror (your.org) to avoid download caps.
  _BASE_URL_TMPL = "https://dumps.wikimedia.your.org/{lang}wiki/{date}/"
  _INFO_FILE = "dumpstatus.json"

  BUILDER_CONFIGS = [
      WikipediaConfig(
          version="0.0.1",
          date="20190220"
      ),
  ]

  def _info(self):
    return tfds.core.DatasetInfo(
        builder=self,
        description=_DESCRIPTION,
        features=tfds.features.FeaturesDict({
            "language":
                tfds.features.Text(),
            "title":
                tfds.features.Text(),
            "content":
                tfds.features.Text(),
        }),
        # No default supervised_keys.
        supervised_keys=None,
        urls=["https://dumps.wikimedia.org"],
        citation=None,
    )

  def _split_generators(self, dl_manager):
    split_generators = []
    for lang in _WIKIPEDIA_LANGUAGES:
      base_url = self._BASE_URL_TMPL.format(
          lang=lang.replace("-", "_"), date=self._builder_config.date)
      info_filepath = dl_manager.download_and_extract(
          base_url + self._INFO_FILE)
      with tf.io.gfile.GFile(info_filepath) as f:
        dump_info = json.load(f)
      multistream_dump_info = dump_info["jobs"]["articlesmultistreamdump"]
      assert multistream_dump_info["status"] == "done", (
          "Specified dump (%s) multistream status is not 'done': %s" % (
              base_url, multistream_dump_info["status"]))

      total_bytes = 0
      urls_to_download = []
      for fname, info in multistream_dump_info["files"].items():
        if ".xml" not in fname:
          continue
        total_bytes += info["size"]
        urls_to_download.append(base_url + fname)

      downloaded_files = dl_manager.download_and_extract(urls_to_download)

      split_generators.append(
          tfds.core.SplitGenerator(
              name=lang,
              num_shards=int(math.ceil(total_bytes / 4 * 2 ** 30)),  # max 4 GB
              gen_kwargs={"filepaths": downloaded_files, "language": lang})
      )
    return split_generators

  def _generate_examples(self, filepaths, language):
    """This function returns the examples in the raw (text) form."""
    for filepath in filepaths:
      logging.info("generating examples from = %s", filepath)
      with tf.io.gfile.GFile(filepath) as f:
        for _, elem in etree.iterparse(f, events=["end"]):
          if not elem.tag.endswith("page"):
            continue
          namespace = elem.tag[:-4]
          title = elem.find("./{0}title".format(namespace)).text
          ns = elem.find("./{0}ns").format(namespace)

          # Filter pages that are not in the "main" namespace.
          if ns != "0":
            continue

          raw_content = elem.find(
              "./{0}revision/{0}text".format(namespace)).text

          # Filter redirects.
          if raw_content.startswith("#REDIRECT"):
            continue

          yield {
              "language": language,
              "title": title,
              "content": _parse_and_clean_wikicode(raw_content)
          }


def _parse_and_clean_wikicode(raw_content):
  """Strips formatting and unwanted sections from raw page content."""
  wikicode = lazy_imports.mwparserfromhell.parse(raw_content)

  # Filters for references, tables, and file/image links.
  re_image_wl = re.compile(
      "^(?:File|Image|Media):", flags=re.IGNORECASE | re.UNICODE)
  bad_template_names = {
      "reflist", "notelist", "notelist-ua", "notelist-lr", "notelist-ur",
      "notelist-lg"}
  bad_tags = {"ref", "table"}
  # These headings denote sections at the end of articles that we do not wish to
  # include. We filter these and do not include any sections that come after.
  stop_headings = {
      "see also", "notes", "further reading", "sources", "references",
      "external links"}

  def is_bad_wikilink(obj):
    return bool(re_image_wl.match(six.text_type(obj.title)))
  def is_bad_tag(obj):
    return six.text_type(obj.tag) in bad_tags
  def is_bad_template(obj):
    return obj.name.lower() in bad_template_names
  def is_stop_heading(heading):
    return heading.title.strip().lower() in stop_headings

  def try_remove_obj(obj, section):
    try:
      section.remove(obj)
    except Exception:  # pylint:disable=broad-except
      pass

  section_text = []
  # Filter individual sections to clean.
  for section in wikicode.get_sections(
      flat=True, include_lead=True, include_headings=True):
    if section.filter_headings(matches=is_stop_heading):
      break
    for obj in section.ifilter_wikilinks(
        matches=is_bad_wikilink, recursive=True):
      try_remove_obj(obj, section)
    for obj in section.ifilter_templates(
        matches=is_bad_template, recursive=True):
      try_remove_obj(obj, section)
    for obj in section.ifilter_tags(matches=is_bad_tag, recursive=True):
      try_remove_obj(obj, section)

    section_text.append(section.strip_code().strip())
  return "\n\n".join(section_text)
